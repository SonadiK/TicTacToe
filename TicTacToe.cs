// Last Modified By:  Sonadi Kannangara  
    ...
    public partial class formTicTacToe : Form
    {
        ...
        private void ClearControls(Control[,] controlArray)
        {
            ...
            foreach (Control controlToClear in controlArray)
            {
                ...
                // Clear the color of winning line.
                controlToClear.BackColor = default(Color);
            }
        }
        ...
        // as long as it's the same!
            ...
            for (int rowCount = 0; rowCount < squares.GetUpperBound(0); rowCount++)
            {
                if (squares[rowCount, 0].Text != String.Empty &&
                    ...
                    squares[rowCount, 1].Text == squares[rowCount, 2].Text &&
                    squares[rowCount, 2].Text == squares[rowCount, 3].Text)
                {
                    // When someone wins, set isWinner = true;.
                    isWinner = true;

                    // The winning line show up in a different colour.
                    squares[rowCount, 0].BackColor = Color.Yellow;
                    squares[rowCount, 1].BackColor = Color.Yellow;
                    squares[rowCount, 2].BackColor = Color.Yellow;
                    squares[rowCount, 3].BackColor = Color.Yellow;
                }
            ...
            for (int colCount = 0; colCount <= squares.GetUpperBound(0); colCount++)
            {
                if (squares[0, colCount].Text != String.Empty &&
                    ...
                    squares[1, colCount].Text == squares[2, colCount].Text &&
                    squares[2, colCount].Text == squares[3, colCount].Text)
                {
                    // When someone wins, set isWinner = true;.
                    isWinner = true;

                    // The winning line show up in a different colour.
                    squares[0, colCount].BackColor = Color.Yellow;
                    squares[1, colCount].BackColor = Color.Yellow;
                    squares[2, colCount].BackColor = Color.Yellow;
                    squares[3, colCount].BackColor = Color.Yellow;
                }
            }
                ...
                squares[0, 0].Text == squares[3, 3].Text)
            {
                ...
                // The winning line show up in a different colour.
                squares[0, 0].BackColor = Color.Yellow;
                squares[1, 1].BackColor = Color.Yellow;
                squares[2, 2].BackColor = Color.Yellow;
                squares[3, 3].BackColor = Color.Yellow;
            }
            else if (squares[0, 3].Text != String.Empty &&
                squares[0, 3].Text == squares[1, 2].Text &&
                squares[1, 2].Text == squares[2, 1].Text &&
                squares[2, 1].Text == squares[3, 0].Text)
            {
                // When someone wins, set isWinner = true;.
                isWinner = true;

                // The winning line show up in a different colour.
                squares[0, 3].BackColor = Color.Yellow;
                squares[1, 2].BackColor = Color.Yellow;
                squares[2, 1].BackColor = Color.Yellow;
                squares[3, 0].BackColor = Color.Yellow;
            }
            ...
            // Make the winning line show up in a different colour! (Done)
            ...
            // Could you make a 4x4 board? Or a 5x5 board? (Done 4x4)
